...
bool Boustrophedon::go_with(PointPtr position, bool flexibility) {
  ...
  for (int i = 1; i * robot_size / 2 < std::abs(position->y - last_position->y);
      i++) {
    new_position = PointPtr(
        new Point(position->x, last_position->y + i * robot_size * flag / 2));
...
bool Boustrophedon::go_into(SpacePtr space, double size_y) {
  ...
  for (int i = 0;
      i
          < int(
              (below_vertices->get_below_point()->x
                  - below_vertices->get_position()->x) / robot_size + EPSILON);
      ++i) {
    ...
    if (i != 0) {
      ...
      if (flag > 0) {
        new_position = PointPtr(
            new Point(intersect_below->x,
                intersect_below->y + space->vary_below));
        go_with(new_position, STRICTLY);
      } else {
        new_position = PointPtr(
            new Point(intersect_upper->x,
                intersect_upper->y - space->vary_upper));
        go_with(new_position, STRICTLY);
      }
    }
    last_position = path.back();
    if (flag > 0) {
      new_position = PointPtr(
          new Point(last_position->x, intersect_upper->y - space->vary_upper));
    } else {
      new_position = PointPtr(
          new Point(last_position->x, intersect_below->y + space->vary_below));
    }
    go_with(new_position, STRICTLY);
    flag = -flag;
  }
  ...
  for (inspectLC = space->children.begin(); inspectLC != space->children.end();
    ...
    if ((*inspectLC)->status_visited == false) {
      ...
      if (map->number_space_need_visit >= 1) {
        //go_with((*inspectLC)->starting_point, STRICTLY);
        go_with(
            PointPtr(
                new Point((*inspectLC)->backtrack_point->x + robot_size,
                    (*inspectLC)->backtrack_point->y)), STRICTLY);
...
std::list<SpacePtr> Boustrophedon::create_list_space(RectanglePtr environment,
  ...
  SegmentPtr segment_upper, segment_below, segment_cut, segment_intersect;
  int i = 1, j = 1;
  ...
  for (inspectLV = list_vertices.begin(), j = 1;
      inspectLV != list_vertices.end(); ++inspectLV) {
    std::cout << "Current Vertices " << j++ << " :("
        << (*inspectLV)->get_position()->x << " ,"
        << (*inspectLV)->get_position()->y << " )\n";
    if ((list_segment.size() == 0) && (inspectLV == list_vertices.begin())) {
      list_segment.push_back(
          SegmentPtr(
              new Segment((*inspectLV)->get_position(),
                  (*inspectLV)->get_upper_point())));
      ++inspectLV;
      list_segment.push_back(
          SegmentPtr(
              new Segment((*inspectLV)->get_position(),
                  (*inspectLV)->get_below_point())));
      continue;
    }
    ...
    segment_cut = SegmentPtr(
        new Segment((*inspectLV)->get_position()->x,
            (*inspectLV)->get_position()->y - environment->get_height(),
            (*inspectLV)->get_position()->x,
            (*inspectLV)->get_position()->y + 2 * environment->get_height()));
    if (!(*inspectLV)->is_obstacles_upper) {
      segment_upper = Vertices::get_segment_contain_nearest_intersect_point(
          list_segment, (*inspectLV)->get_position(),
          environment->get_height());
    }
    if (!(*inspectLV)->is_obstacles_below) {
      segment_below = Vertices::get_segment_contain_nearest_intersect_point(
          list_segment, (*inspectLV)->get_position(),
          -environment->get_height());
    }
    ...
    case 1:
      ...
      if (!(*inspectLV)->is_obstacles_upper) {
        ...
        list_point_of_space.push_back(
            Vertices::get_point_x_litte(segment_upper));
        list_point_of_space.push_back(segment_upper % segment_cut);
        list_point_of_space.push_back((*inspectLV)->get_position());
        list_space.push_back(
            SpacePtr(new Space(list_point_of_space, robot_size)));
        list_point_of_space.clear();
        Vertices::update_list_vertices(list_vertices,
            Vertices::get_point_x_large(segment_upper),
            Vertices::get_point_x_litte(segment_upper),
            segment_cut % segment_upper);
        Vertices::update_list_segment(list_segment, segment_upper,
            segment_upper % segment_cut);
        list_point_of_space.push_back(
            Vertices::get_point_x_litte(segment_below));
        list_point_of_space.push_back((*inspectLV)->get_below_point());
        list_point_of_space.push_back((*inspectLV)->get_position());
        list_point_of_space.push_back(segment_below % segment_cut);
        Vertices::update_list_vertices(list_vertices,
            Vertices::get_point_x_large(segment_below),
            Vertices::get_point_x_litte(segment_below),
            segment_cut % segment_below);
        Vertices::update_list_segment(list_segment, segment_below,
            segment_below % segment_cut);
      } else {
        list_point_of_space.push_back((*inspectLV)->get_below_point());
        list_point_of_space.push_back((*inspectLV)->get_upper_point());
        list_point_of_space.push_back((*inspectLV)->get_position());
      ...
      list_segment.remove(
          SegmentPtr(
              new Segment((*inspectLV)->get_below_point(),
                  (*inspectLV)->get_position())));
      list_segment.remove(
          SegmentPtr(
              new Segment((*inspectLV)->get_upper_point(),
                  (*inspectLV)->get_position())));
      break;
    case 2:
      std::cout << "Type Vertices :" << (*inspectLV)->type_vertice << "\n";
      ...
      Vertices::update_list_vertices(list_vertices,
          Vertices::get_point_x_large(segment_intersect),
          Vertices::get_point_x_litte(segment_intersect),
          segment_cut % segment_intersect);
      Vertices::update_list_segment(list_segment, segment_intersect,
          segment_intersect % segment_cut);
      list_segment.remove(
          SegmentPtr(
              new Segment((*inspectLV)->get_below_point(),
                  (*inspectLV)->get_position())));
    ...
    case 5:
      ...
      if (!(*inspectLV)->is_obstacles_below) {
        ...
        if (!(*inspectLV)->is_obstacles_upper) {
          ...
          temp_vertices = VerticesPtr(
              new Vertices((*inspectLV)->get_position(),
                  (*inspectLV)->get_below_point(),
          ...
          if ((*inspectLV)->is_obstacles_below) {
            ...
            segment_intersect =
                Vertices::get_segment_contain_nearest_intersect_point(
                    list_segment, (*inspectLV)->get_position(),
                    -environment->get_height());
            list_point_of_space.push_back(
                Vertices::get_point_x_litte(segment_intersect));
            list_point_of_space.push_back((temp_vertices)->get_upper_point());
            list_point_of_space.push_back((temp_vertices)->get_position());
            list_point_of_space.push_back(segment_intersect % segment_cut);
          }
        }
      }
      Vertices::update_list_vertices(list_vertices,
    ...
    case 6:
      ...
      temp_vertices = VerticesPtr(
          new Vertices((*inspectLV)->get_position(),
              (*inspectLV)->get_below_point(), (*inspectLV)->get_upper_point(),
              true));
      ++inspectLV;
      if ((*inspectLV)->type_vertice == 6) {
        if (!temp_vertices->is_obstacles_upper) {
          segment_below = Vertices::get_segment_contain_nearest_intersect_point(
              list_segment, (*inspectLV)->get_position(),
              -environment->get_height());
          segment_upper = Vertices::get_segment_contain_nearest_intersect_point(
              list_segment, (temp_vertices)->get_position(),
              environment->get_height());
          list_point_of_space.push_back(
              Vertices::get_point_x_litte(segment_below));
          list_point_of_space.push_back(
        ...
        if (((*inspectLV)->get_below_point()->x
            - (*inspectLV)->get_position()->x) > EPSILON) {
          temp_point = (*inspectLV)->get_below_point();
        } else {
          temp_point = (*inspectLV)->get_upper_point();
        }
        list_segment.push_back(
            SegmentPtr(new Segment((*inspectLV)->get_position(), temp_point)));
      }
      if ((*inspectLV)->type_vertice == 5) {
        if (((*inspectLV)->get_position()->x
            - (*inspectLV)->get_below_point()->x) > EPSILON) {
          temp_point = (*inspectLV)->get_below_point();
        } else {
          temp_point = (*inspectLV)->get_upper_point();
        }
        list_segment.remove(
            SegmentPtr(new Segment(temp_point, (*inspectLV)->get_position())));
        if (temp_vertices->is_obstacles_upper) {
          segment_intersect =
              Vertices::get_segment_contain_nearest_intersect_point(
                  list_segment, (*inspectLV)->get_position(),
      ...
      if ((temp_vertices->get_below_point()->x
          - (temp_vertices)->get_position()->x) > EPSILON) {
        temp_point = temp_vertices->get_below_point();
      } else {
        temp_point = temp_vertices->get_upper_point();
      }
      list_segment.push_back(
          SegmentPtr(new Segment(temp_vertices->get_position(), temp_point)));
      break;
    default:
      std::cout << "\nType Vertices :" << (*inspectLV)->type_vertice << "\n";
      break;
  ...
  for (inspectLS = --list_space.end(), i = 1; inspectLS != list_space.end();
    ...
    for (inspectLS_temp = list_space.begin(), i = 1;
      ...
      if (Space::is_parent(*inspectLS_temp, *inspectLS)) {
        ...
        (*inspectLS)->set_point_backtrack(*inspectLS_temp, robot_size,
            environment->get_height());
        break;
      }
    }
...
void Boustrophedon::boustrophedon_cd() {
  ...
  Vertices::print_info_list_vertices(list_vertices);
}
}
